...
const logger = config.logger.child({ name: 'controllers.role' });

class RoleController {
  /**
   * Find user for role
   */
  static async userForRole(role) {
   ...
   */
  static async playerForRole(role) {
    return await models.User.findOne({
      where: {
        isActive: true,
        isArchived: false
   ...
   * Assign user to role
   */
  static async assignUserToRole(user, role) {
