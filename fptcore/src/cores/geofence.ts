...
class GeofenceCore {
  static isOverlappingGeofence(scriptContent: any, latitude: number, longitude: number, accuracy: number, waypointOptions: any,
    geofence: any): boolean {
   ...
   */
  static geofencesInArea(scriptContent: any, latitude: number, longitude: number, accuracy: number, waypointOptions: any): any[] {
    if (!latitude || !longitude) {
      return [];
    }
    const geofences = scriptContent.geofences || [];
    return geofences.filter((g: any) => this.isOverlappingGeofence(scriptContent,
