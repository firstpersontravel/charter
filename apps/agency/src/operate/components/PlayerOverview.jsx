...
function renderUser(player, user) {
  ...
  return (
    <Link
      to={
        `/${player.org.name}/${player.experience.name}/` +
  ...
  return (
    <div>
      <div>
        <strong>Role:</strong>
        &nbsp;
        <Link to={`/${player.org.name}/${player.experience.name}/operate/${trip.groupId}/role/${player.roleName}/${user ? user.id : 0}`}>
