...
function renderSidebar() {
  const renderedModules = Object.keys(coreRegistry.modules).map(moduleName => (
    renderSidebarModule(moduleName, coreRegistry.modules[moduleName])
  ));
  return (
...
function renderResourceField(key, spec, prefix) {
  ...
  if (spec.type === 'list') {
    return [
      ...
      renderResourceField('item', spec.items, childPrefix)
    ];
  }
  if (spec.type === 'dictionary') {
    return [
      renderResourceFieldItem(key, spec, prefix),
      renderResourceField('key', spec.keys, childPrefix),
      renderResourceField('value', spec.values, childPrefix)
    ];
  }
  if (spec.type === 'object') {
    return [renderResourceFieldItem(key, spec, prefix)]
      .concat(...Object.keys(spec.properties).map(k => (
        renderResourceField(k, spec.properties[k], childPrefix)
      )));
  }
  return renderResourceFieldItem(key, spec, prefix);
}
...
function renderMain() {
  const renderedModules = Object.keys(coreRegistry.modules).map(moduleName => (
    renderModule(moduleName, coreRegistry.modules[moduleName])
  ));
  return (
...
export default class HelpReference extends Component {
  render() {
    return (
